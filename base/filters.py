...
from .models import Site, Notification
...
class SiteListFilter(df.FilterSet):
    name = df.CharFilter(widget=forms.TextInput(attrs={'placeholder': 'Nom'}), lookup_expr='icontains')
...
class NotificationListFilter(df.FilterSet):
    created_at= MaeDateRangeFilter(field_name="created__date", widget=forms.TextInput(attrs={'placeholder': 'Date', "class": "date-range-picker"}))
    # read_at = df.BooleanFilter(widget=forms.CheckboxInput(attrs={'class': 'form-check-input'}), label='Lue')
    class Meta:
        model = Notification
        fields = ['created_at',]